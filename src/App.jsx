        ...
        <div style={{display:"flex",gap:6,marginBottom:7}}>
          ...
          {SEAS.map(s=>(
            <button key={s} onClick={()=>{
              ...
              style={{flex:1,padding:"8px 0",borderRadius:9,border:"none",cursor:"pointer",fontSize:13.5,fontWeight:800,minWidth:0,
          ...
          <div style={{position:"relative",flex:3,minWidth:0}}>
            ...
            {ptOpen && ptQuery && (
              <div style={{position:"absolute",top:"calc(100% + 4px)",right:0,width:220,maxHeight:280,overflowY:"auto",
        ...
        <div style={{background:"rgba(255,255,255,0.12)",borderRadius:12,padding:"5px",marginBottom:6,maxHeight:"calc(100vh - 250px)",overflowY:"auto"}}>
          {REGIONS.filter(r=>r.sea===selSea).map(r=>(
            <button key={r.id} onClick={()=>setRegion(r)}
              ...
              <span>{r.kind==="원도"?"🚢":"📍"}</span>
              {r.name}
              {r.kind==="원도" && <span style={{fontSize:10.5,color:r.id===region.id?C.gray:"rgba(255,255,255,0.7)",marginLeft:"auto"}}>먼바다</span>}
            </button>
          ))}
        </div>
